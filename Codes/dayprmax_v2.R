...
if (exists("day.mean_1") == FALSE){
  for (i in sequence){  # Problem with 20 
    ...
    rad <- circular(day.prmax / 365 * 2*pi)
    #rad <- apply(day.prmax, c(1,2), circular) #Erst von Tagen in pi umrechnen dann circular applyien
    mean.tmp <- apply(rad, c(1,2), mean)
    sd.tmp <- apply(rad, c(1,2), sd)
    mean.tmp <- ifelse(mean.tmp <  0, mean.tmp + 2*pi , mean.tmp)
    sd.tmp <- ifelse(sd.tmp <  0, sd.tmp + 2*pi , sd.tmp)
    mean.tmp <- (mean.tmp / 6.283185) * 365 #Wieder in Tagen umrechnen
    sd.tmp <- (sd.tmp / 6.283185) * 365
    assign(paste0("day.mean_", i), mean.tmp)
    assign(paste0("day.sd_", i), sd.tmp)  #Sd is positiv definiert also andere colorbar verwenden
  }
}
...
for (names in obvs_data){
  ...
  rad <- circular(day.prmax/ 365 * 2*pi)
  mean.tmp <- apply(rad, c(1,2), mean)
  mean.tmp <- ifelse(mean.tmp < 0.02, NA, mean.tmp)  #Change NA Values to NA values
  #idx <- which(mean_obvs < 0.02)
  sd.tmp <- apply(rad, c(1,2), sd)
  mean.tmp <- ifelse(mean.tmp <  0, mean.tmp + 2*pi , mean.tmp)
  sd.tmp <- ifelse(sd.tmp <  0, sd.tmp + 2*pi , sd.tmp)
  mean_obvs <- (mean.tmp / 6.283185) * 365
  std_obvs <- (sd.tmp / 6.283185) * 365
  lat <- lat
  lon <- lon
  #Change 0 values to NA values if necessary
  mean_obvs[mean_obvs == 1] = NA
  ...
  dimnames(sd.new.obvs) = list(lon = lon.new, lat=lat.new)
  ...
  for (i in sequence){
    ...
    load(gsub(" ", "", (paste("./prec/hist_",model_his[i],".prmax.RData"))))
    prmax.his = get(paste("day.mean_", i, sep = ""))
    ...
    diff.df <- reshape2::melt(diff.matrix)
  ...
  #To make a statement why prmax has such extreme values and dayprmax does not
  lis_diff_new <- list()
  ...
  assign(paste0("day_mean_diff_",  names), mean_lis_diff)
  print(mean_lis_diff)
  print("The maximum of lis_diff_new is")
  print(max(unlist(lis_diff_new)))
  assign(paste0("day_max_diff_",  names), max(unlist(lis_diff_new)))
  print("The minimum of lis_diff_new is")
  print(min(unlist(lis_diff_new)))
  assign(paste0("day_min_diff_",  names), min(unlist(lis_diff_new)))
  
  mean_lis_sd <- mean(unlist(lis_sd_new), na.rm = TRUE)
  print("The mean of the list of sds is calculated now")
  print(mean_lis_sd)
  assign(paste0("day_mean_sd_",  names), mean_lis_sd)
  print("The maximum of lis_diff_new is")
  print(max(unlist(lis_sd_new)))
  assign(paste0("day_max_diff_",  names), max(unlist(lis_sd_new)))
  print("The minimum of lis_diff_new is")
  print(min(unlist(lis_sd_new)))
  assign(paste0("day_min_diff_",  names), min(unlist(lis_sd_new)))
  ...
  ggsave(path = paste0("~/Projekt_22/google/day.prmax/", names, sep="","/") , filename = paste0(names, sep="", "_vs_hist_mean_FINAL.png"))
  ...
  ggsave(path = paste0("~/Projekt_22/google/day.prmax/", names, sep="","/") , filename = paste0(names, sep="", "_vs_hist_std_FINAL.png"))
  print("Figure saved SD")
  dev.set(dev.prev())
  dev.off()
